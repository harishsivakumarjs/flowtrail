...
import { Plus, ChevronRight, Flame, Moon, CheckSquare2, Zap } from 'lucide-react'
...
function StatCard({ label, value, sub, color, icon: Icon }) {
  return (
    <div className="rounded-xl p-4 flex flex-col gap-1"
      ...
      <div className="text-2xl font-semibold mt-0.5" style={{ color: 'var(--text-primary)' }}>
        {value}
      </div>
...
export default function Dashboard() {
  ...
  const logMap = Object.fromEntries(
    (todayLogs || []).map(l => [l.habit_id, l])
  )
  const doneCount = (todayLogs || []).filter(l => l.completed).length

  // Mini heatmap: last 30 days of habit completion
  ...
  const highPriority = todayPendingTasks.filter(t => t.priority === 'high')
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <Link to="/journal" className="btn btn-primary text-sm">
          <Plus size={15} /> Write today
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          sub={highPriority.length > 0 ? `${highPriority.length} high priority` : 'All clear'}
          icon={CheckSquare2}
          color="var(--amber)"
        />
        <StatCard
          label="Sleep"
          value={sleepLog ? `${sleepLog.hours}h` : '—'}
          sub={sleepLog ? (sleepLog.hours >= 7 ? 'Above goal ✓' : 'Below 7h goal') : 'Not logged'}
          icon={Moon}
          color="var(--brand-light)"
        />
        <StatCard
          label="Best streak"
          value={habits.length ? '—' : '0d'}
          sub="Track to see"
          icon={Flame}
          color="var(--red)"
        />
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="card">
          <div className="flex items-center justify-between px-4 py-3 border-b"
            ...
            <Link to="/habits" className="text-xs flex items-center gap-1"
          ...
          <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
            {habits.length === 0 && (
              <div className="px-4 py-8 text-center">
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>No habits yet</p>
                <Link to="/habits" className="btn btn-primary text-sm mt-3 inline-flex">
            ...
            {habits.slice(0, 7).map(habit => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${done ? 'border-[var(--green)] bg-[var(--green)]' : 'border-[var(--border-mid)]'}`}
                  >
                    {done && (
        ...
        <div className="space-y-4">
          ...
          <div className="card px-4 py-3">
            ...
            <div className="flex items-center gap-1.5 mt-2">
              ...
              {[0,1,2,3,4].map(l => (
                <div key={l} className={`w-3 h-3 rounded-sm heat-${l}`} />
              ))}
